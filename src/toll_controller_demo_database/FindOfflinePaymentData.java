...
public class FindOfflinePaymentData extends JFrame implements ActionListener {

    JScrollPane ScrollPanel;
    Choice OfflinePayment;
    JButton SearchButton, TablePrintButton, UpdateButton, backButton;
    JTable DataTable; //table


    public FindOfflinePaymentData() {
        ...
        //Search
        JLabel Search = new JLabel("Search by Date and Time:");
        Search.setBounds(50, 75, 250, 35);
        Search.setFont(new Font("serif", Font.BOLD, 20));
        add(Search);


        OfflinePayment = new Choice();
        OfflinePayment.setBounds(300, 83, 250, 60);
        add(OfflinePayment);
        ...
        try {
            ...
            ResultSet AllData = ccc.s.executeQuery("select * from collected_toll_record");
            while(AllData.next()){
                OfflinePayment.add(AllData.getString("Payment_Gateway")); //find by
            }
    ...
    public void actionPerformed (ActionEvent ae) {
        ...
        if(ae.getSource() == SearchButton){
            //setVisible(false);
            String EmployeeSearchQuary = " select * from collected_toll_record where Payment_Gateway = '"+ OfflinePayment.getSelectedItem()+"'";

            try {
                Conn ccc = new Conn();
                ResultSet Data = ccc.s.executeQuery(EmployeeSearchQuary);
